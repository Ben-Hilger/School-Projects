...
void doWorkerTasks() {
    ...
    do {
        /* read the two numbers from the manager (rank == 0) */
        world.recv(MANAGER_RANK, NUMBER_TAG, numbers);
        // Check if -1 isn't in the list
        if (numbers[0] != -1) {
            // Store the factors
            std::vector<int> factorList;
            // Get the factors of the two numberss
            factorList.push_back(getFactorCount(numbers[0]));
            factorList.push_back(getFactorCount(numbers[1]));
            /* send number of factors back to the manager */
            world.send(MANAGER_RANK, FACTORS_TAG, factorList);
        }
    } while (numbers[0] != -1);
}
...
void doManagerTasks(const std::string& path) {
    ...
    if (!data) {
        ...
        // Store the two current numbers
        std::vector<long long> numbers(2);
        // Keep reading pairs of values from the data file
        while (data >> numbers[0] >> numbers[1]) {
            // Send numbers to be converted to the worker
            world.send(WORKER_RANK, NUMBER_TAG, numbers);
            // Receive factors from the worker
            std::vector<int> factors;
            world.recv(WORKER_RANK, FACTORS_TAG, factors);
            for (size_t i = 0; i < numbers.size(); i++) {
                // Print results
                std::cout << "Number " << numbers[i] << " has " << factors[i]
                        << " factors.\n";
            }
        }
    }
    // send an empty vector to worker to inform worker to end processing
    std::vector<long long> numbers = {-1, -1};
    world.send(WORKER_RANK, NUMBER_TAG, numbers);
}
