...
void NeuralNet::learn(const Matrix& inputs, const Matrix& expected,
    ...
    MatrixVec act = { inputs }, zs;
    // Do the forward propagation layer-by-layer
    for (size_t lyr = 0; (lyr < biases.size()); lyr++) {
        zs.push_back(weights[lyr].dot(activation, mat) + biases[lyr]);
    ...
    auto delta = (act.back() - expected) * zs.back().apply(invSigmoid);
    ...
    const int lastLyr = layerSizes.width() - 1;
    nabla_w.push_back(delta.dot(act.at(lastLyr - 1).transpose(mat), mat));
    ...
    for (auto lyr = 2; (lyr <= lastLyr); lyr++) {
        ...
        delta = weights[lastLyr - lyr + 1].transpose(mat).dot(delta, mat) * sp;
        nabla_b.push_back(delta);
        nabla_w.push_back(delta.dot(act[lastLyr - lyr].transpose(mat), mat));
    }
    ...
    */
    ...
    for (auto lyr = 0, revLyr = lastLyr - 1; (lyr < lastLyr); lyr++, revLyr--) {
        weights[lyr].subtract(nabla_w[revLyr].multi(eta));
        biases[lyr].subtract(nabla_b[revLyr].multi(eta));
    }
}
...
std::istream& operator>>(std::istream& is, NeuralNet& nnet) {
    ...
    is >> nnet.layerSizes;
    const int layerCount = nnet.layerSizes.width();
...
Matrix
NeuralNet::classify(const Matrix& inputs) {
    Matrix result = inputs;
    for (size_t lyr = 0; (lyr < weights.size()); lyr++) {
        result = (weights[lyr].dot(result, mat) + biases[lyr], mat)
            .apply(sigmoid);
    }
    return result;
}
