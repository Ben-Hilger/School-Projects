...
Matrix getExpectedDigitOutput(const std::string& path) {
    ...
    expected.set(label, 0, 1.0);  // Just label should be 1.0
    return expected;
}
...
void assess(NeuralNet& net, const std::string& path,
    ...
    for (std::string imgName; std::getline(fileList2, imgName); totCount++) {
        ...
        const int expIdx = maxElemIndex(exp.transpose(mat).getRow(0));
        const int resIdx = maxElemIndex(res.transpose(mat).getRow(0));
        if (expIdx == resIdx) {
            passCount++;
        }
